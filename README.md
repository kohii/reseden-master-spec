...
`uv run scripts/cli.py verify` で抽出結果を機械検査する。
終了コードが `0` なら OK、`1` なら何らかの error を検出。

```bash
# 最新版を検査
uv run scripts/cli.py verify

# 前版との差分（新規/削除マスター、fieldCount 変動）
uv run scripts/cli.py verify --baseline 20260331
```
...
- `_recover_ranged_fields()` は `10〜99` のような範囲ヘッダを検出して展開する。
  1孤立子行に全サブが乗らないケース（shika_ケ 90項目）は未対応。
- `scripts/text_supplement.py` は `pdftotext -layout` で pdfplumber の取りこぼしを補完する
